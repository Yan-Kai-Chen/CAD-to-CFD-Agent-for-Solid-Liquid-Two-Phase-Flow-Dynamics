# Documentation Index

This project is best read as four connected workflow blocks: `Modeling`,
`FastFluent`, `Meshing`, and `Fluent`.

Start with the project-level documents:

- `architecture.md`
- `public_asset_framework_map.md`
- `installation.md`
- `safety_policy.md`
- `skills.md`

## Paper-Facing Agent Route

- `public_asset_framework_map.md`
- `agent_benchmark_ladder/README.md`
- `dewaxing_agent/README.md`
- `AGENT_BENCHMARK_LADDER_REPLAN_20260628.md`
- `DEWAXING_AGENT_GITHUB_RESTRUCTURE_HANDOFF_20260628.md`

Use these documents first when the goal is to understand how the original
public GitHub assets were split into the current framework and paper-facing
route.

## Modeling

- `nx/overview.md`
- `nx/quickstart.md`
- `nx/reverse_modeling_workflow.md`
- `nx/manual_journal_capture.md`
- `solidworks/overview.md`
- `solidworks/quickstart.md`
- `solidworks/workflow.md`
- `mesh/freecad_solidify.md`

## FastFluent

- `../cpp/fastfluent_core/README.md`
- `agent_benchmark_ladder/README.md`
- `dewaxing_agent/README.md`
- `fastcfd/AGENT_WORKFLOW_STATUS_AUDIT.md`
- `fastcfd/CURRENT_CAPABILITY_SNAPSHOT.md`
- `fastcfd/CASESPEC_V3.md`
- `fastcfd/EVIDENCE_BUNDLE_V3.md`
- `fastcfd/BOUNDARY_AND_MATERIAL_CONTRACTS.md`
- `fastcfd/MESH_GATEWAY_V2.md`
- `fastcfd/FLOW_PACK_ADAPTER.md`
- `fastcfd/ROUTE_SELECTOR.md`
- `fastcfd/ROUTE_PLAN_COMPILER.md`
- `fastcfd/EXECUTION_GATE.md`
- `fastcfd/CONTROLLED_RUNNER.md`
- `fastcfd/RESULT_PACK_COMPILER.md`
- `fastcfd/SOLVER_CAPABILITY_MATRIX.md`
- `fastcfd/TRANSPORT_COUPLING_CORE.md`
- `fastcfd/WORKFLOW_RUNNER.md`
- `fastcfd/DEWAXING_APPLICATION_BRIDGE.md`
- `DEWAXING_FASTFLUENT_TO_FLUENT_ASSET_INVENTORY_20260628.md`
- `fastcfd/SERVER_CODEX_DEPLOYMENT_RUNBOOK.md`
- `fastcfd/MOTION_CONTRACT.md`
- `fastcfd/quickstart.md`
- `fastcfd/fastfluent_baseline_20260613.md`
- `fastcfd/lattice_trust_and_pilot_decision.md`
- `fastcfd/unstructured_mesh_gateway.md`
- `fastcfd/physics_passports_and_fluent_hints.md`
- `fastcfd/vof_physics_passport.md`
- `FASTFLUENT_HORIZONTAL_H1_EVIDENCE_ADAPTER_INVENTORY_20260622.md`
- `FASTFLUENT_HORIZONTAL_H1_DELIVERY_20260622.md`
- `FASTFLUENT_HORIZONTAL_H2_STEAM_AIR_CONDENSATION_V2_EVIDENCE_ADAPTER_INVENTORY_20260623.md`
- `FASTFLUENT_HORIZONTAL_H2_STEAM_AIR_CONDENSATION_V2_DELIVERY_20260623.md`
- `FASTFLUENT_HORIZONTAL_H3_PROGRESS_20260623.md`
- `FASTFLUENT_HORIZONTAL_H3_DELIVERY_20260623.md`
- `FASTFLUENT_HORIZONTAL_H3_5_VALIDATION_PACK_GOAL_20260623.md`
- `FASTFLUENT_HORIZONTAL_H3_5_PROGRESS_20260623.md`
- `FASTFLUENT_HORIZONTAL_H3_5_DELIVERY_20260623.md`
- `FASTFLUENT_HORIZONTAL_H4_WAX_RHEOLOGY_PHASE_CHANGE_GOAL_20260623.md`
- `FASTFLUENT_HORIZONTAL_H4_WAX_RHEOLOGY_PHASE_CHANGE_PROGRESS_20260623.md`
- `FASTFLUENT_HORIZONTAL_H4_WAX_RHEOLOGY_PHASE_CHANGE_DELIVERY_20260623.md`
- `FASTFLUENT_S1_NATIVE_SIMULATION_VALIDATION_PACK_GOAL_20260623.md`
- `FASTFLUENT_S1_NATIVE_SIMULATION_PROGRESS_20260623.md`
- `FASTFLUENT_S1_NATIVE_SIMULATION_DELIVERY_20260623.md`
- `FASTFLUENT_S2_PRACTICAL_NATIVE_FUNCTION_EXPANSION_GOAL_20260623.md`
- `FASTFLUENT_S2_PRACTICAL_NATIVE_FUNCTION_EXPANSION_PROGRESS_20260623.md`
- `FASTFLUENT_S2_PRACTICAL_NATIVE_FUNCTION_EXPANSION_DELIVERY_20260623.md`
- `FASTFLUENT_S3_PRACTICAL_NATIVE_SETUP_UTILITIES_PROGRESS_20260623.md`
- `FASTFLUENT_S3_PRACTICAL_NATIVE_SETUP_UTILITIES_DELIVERY_20260623.md`
- `FASTFLUENT_DEWAXING_NATIVE_SOLVER_PROGRESS_20260627.md`

## Meshing

- `hypermesh_meshing/interface_draft.md`
- `hypermesh_meshing/local_execution_adapter.md`
- `fluent_meshing/interface_draft.md`
- `fluent_meshing/roadmap.md`

## Fluent

- `fluent_solver/interface_draft.md`
- `fluent_solver/local_execution_adapter.md`
- `fluent_solver/solver_plan_contract.md`
- `fluent_solver/resume_and_monitoring.md`
- `fluent_solver/roadmap.md`
- `postprocessing/interface_draft.md`
- `postprocessing/monitor_contract.md`
- `postprocessing/video_and_metrics_contract.md`
- `postprocessing/roadmap.md`
