...
int main() {
  ...
  // ------------------ define lattice ------------------
  using NSFIELDS = TypePack<RHO<T>, VELOCITY<T, LatSet::d>, POP<T, LatSet::q>, CONSTFORCE<T, LatSet::d>, OMEGA<T>, SMAGORINSKY<T>>; //, StrainRateMag<T>
  ...
  ValuePack NSInitValues(BaseConv.getLatRhoInit(), Vector<T, 2>{}, T{}, Vector<T, 2>{T{}, -BaseConv.Lattice_g}, BaseConv.getOMEGA(), Smagorinsky); //, T{}
  ...
  // nu = m * gamma^(n-1)
  // [mm^2 s^-1] = [mm^2 s^-1 s^(n-1)] * [s^(1-n)]
  const T latvis = BaseConv.Lattice_VisKine;
  ValuePack PowerLawInitValues(latvis / std::pow(BaseConv.Conv_Time, BehaviorIndex - 1),
  BehaviorIndex - 1, latvis*MInViscCoef, latvis*MaxViscCoef);
  ...
  using NSBulkTask =
    tmp::Key_TypePair<olbfs::FSType::Fluid | olbfs::FSType::Interface | olbfs::FSType::Gas,
                      collision::PowerLaw_BGKForce<moment::forceRhou<NSCELL, force::ConstForce<NSCELL>, true>,
  ...
  // vtmo::ScalarWriter GammaWriter("Gamma", NSLattice.getField<StrainRateMag<T>>());
