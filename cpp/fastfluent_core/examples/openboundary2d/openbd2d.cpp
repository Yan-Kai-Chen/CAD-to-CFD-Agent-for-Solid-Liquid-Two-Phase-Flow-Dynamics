...
int main() {
  ...
  while (MainLoopTimer() < MaxStep && res > tol) {
    ...
    // use Communicate() (less efficient) instead,
    ...
    // NSLattice.getField<POP<T, LatSet::q>>().Communicate(MainLoopTimer());
  ...
  Printer::Endl();
  WriteFastFluentNativeSummary("channel2d", MainLoopTimer(), MaxStep, OutputStep, res,
                               Ni, Nj, Cell_Len, rho_ref, Kine_Visc, U_Max,
                               BaseConv.getPhysTime(MainLoopTimer()), "cavblock2d");
