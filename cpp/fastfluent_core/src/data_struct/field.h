...
// Windows native builds do not provide POSIX shm_open/mmap. For non-CUDA
// agent-side pilot runs, fall back to StreamArray so controlled examples can
// compile without requiring an ad hoc sys/mman.h compatibility header.
template <typename T>
class StreamMapArray : public StreamArray<T> {
 public:
  using value_type = T;
  using array_type = StreamMapArray<T>;

  StreamMapArray() : StreamArray<T>() {}
  explicit StreamMapArray(std::size_t size) : StreamArray<T>(size) {}
  StreamMapArray(std::size_t size, T InitValue) : StreamArray<T>(size, InitValue) {}
};
